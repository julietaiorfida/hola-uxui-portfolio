...
const Experience = () => {
  ...
  return (
    <section id="experience" className="py-12 px-6 bg-[hsl(var(--section-rosa-mid))] border-t border-border/30 relative overflow-hidden">
      <div className="max-w-7xl mx-auto">
        <div ref={titleRef}>
          <h2 className="text-3xl sm:text-4xl md:text-5xl font-black text-primary mb-10 text-center md:text-left">
            {t("exp.title")}
          </h2>
        </div>

        <div className="grid md:grid-cols-1 lg:grid-cols-[1fr_1fr] gap-8 items-start">
          <div className="space-y-8">
            {/* Timeline */}
            <div ref={timelineRef} className="space-y-5">
              {experiences.map((exp, index) => (
                <div key={index} data-reveal-item className="relative pl-6 border-l-4 border-primary">
                  <div className="absolute -left-2.5 top-0 w-5 h-5 rounded-full bg-primary"></div>
                  <h3 className="text-lg font-bold text-foreground mb-0.5">{t(exp.roleKey)}</h3>
                  <p className="text-base text-primary font-semibold mb-1">{t(exp.companyKey)}</p>
                  <p className="text-xs text-muted-foreground mb-1">{t(exp.periodKey)}</p>
                  <p className="text-sm text-foreground/80">{t(exp.descKey)}</p>
                </div>
              ))}
            </div>

            {/* Skills */}
            <div ref={skillsRef} className="grid md:grid-cols-2 gap-5">
              {[skillsCol1, skillsCol2].map((col, ci) => (
                <div key={ci}>
                  {ci === 0 && <h3 className="text-xl font-bold text-primary mb-3">{t("exp.skillsTitle")}</h3>}
            ...
            {/* Tools */}
            <div>
              <h3 className="text-xl font-bold text-primary mb-3">{t("exp.toolsTitle")}</h3>
              <div ref={toolsRef} className="grid grid-cols-4 sm:grid-cols-6 gap-3">
          ...
          <div className="space-y-5">
            <div ref={eduRef} className="bg-background rounded-2xl p-5 shadow-lg border-2 border-primary/20">
              <h3 className="text-xl font-bold text-primary mb-4">{t("exp.eduTitle")}</h3>
              <div className="space-y-4">
                <div>
                  <p className="text-base font-semibold text-accent mb-0.5">{t("exp.edu1.school")}</p>
                  <p className="text-sm text-foreground/80 mb-0.5">{t("exp.edu1.degree")}</p>
                  <p className="text-xs text-muted-foreground">{t("exp.edu1.period")}</p>
                </div>
                <div>
                  <p className="text-base font-semibold text-accent mb-0.5">{t("exp.edu2.school")}</p>
                  <p className="text-sm text-foreground/80 mb-0.5">{t("exp.edu2.degree")}</p>
                  <p className="text-xs text-muted-foreground">{t("exp.edu2.period")}</p>
                </div>
              </div>
            </div>

            <div ref={certRef} className="bg-background rounded-2xl p-5 shadow-lg border-2 border-primary/20">
              <h3 className="text-xl font-bold text-primary mb-4">{t("exp.certTitle")}</h3>
              <div className="space-y-4">
                <div>
                  <p className="text-base font-semibold text-accent mb-0.5">{t("exp.cert1.name")}</p>
                  <p className="text-sm text-foreground/80 mb-0.5">{t("exp.cert1.org")}</p>
                  <p className="text-xs text-muted-foreground">{t("exp.cert1.date")}</p>
                </div>
                <div>
                  <p className="text-base font-semibold text-accent mb-0.5">{t("exp.cert2.name")}</p>
            ...
            <div ref={volRef} className="bg-background rounded-2xl p-5 shadow-lg border-2 border-primary/20">
              <h3 className="text-xl font-bold text-primary mb-4">{t("exp.volTitle")}</h3>
              <div>
                <p className="text-base font-semibold text-accent mb-0.5">{t("exp.vol.org")}</p>
                <p className="text-xs text-muted-foreground mb-2">{t("exp.vol.period")}</p>
                <p className="text-sm text-foreground/80">{t("exp.vol.desc")}</p>
              </div>
            </div>
          </div>
        </div>
