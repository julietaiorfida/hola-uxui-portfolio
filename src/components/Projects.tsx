...
const Projects = () => {
  ...
  return (
    <section id="projects" className="min-h-screen py-20 px-6 bg-gradient-to-b from-accent/5 via-background to-primary/5 relative overflow-hidden" ref={ref}>
      <div className={`max-w-7xl mx-auto ${isVisible ? 'scroll-fade-in' : 'opacity-0'}`}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {projects.map((project, index) => (
            <div key={index} className={`${project.color} text-white rounded-3xl p-8 shadow-2xl hover:shadow-xl transition-all hover:scale-105 flex flex-col animate-fade-in`} style={{ animationDelay: `${index * 0.1}s` }}>
